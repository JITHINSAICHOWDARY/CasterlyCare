...
// Cloud Sky (Originkit), ported to plain JS/Vite — a normal daytime sky-blue
// palette. Capped at 1x pixel density (not the original's 2x): this shader
// is genuinely expensive per-pixel (nested noise loops across 3 cloud
// layers), and it runs alongside the skeleton's own WebGL canvas — 2x
// density on a retina display means 4x the fragment-shader work for a sky
// that reads the same either way, and was the main cause of the lag.
const MAX_DPR = 1;
...
function compile(gl, type, src) {
  ...
  if (!gl.getShaderParameter(sh, gl.COMPILE_STATUS)) {
    console.warn('CloudSky shader:', gl.getShaderInfoLog(sh)); // eslint-disable-line no-console
    gl.deleteShader(sh);
    return null;
  }
  return sh;
...
function parseColor(input, fb) {
  ...
  if (str.charAt(0) === '#') {
    ...
    if (hex.length >= 6) {
      ...
      if (!Number.isNaN(r) && !Number.isNaN(g) && !Number.isNaN(b)) return [r / 255, g / 255, b / 255, a];
    }
    return fb;
  }
...
function CloudSkyBase(props) {
  ...
  useEffect(() => {
    ...
    if (!gl) {
      console.warn('CloudSky: WebGL unavailable'); // eslint-disable-line no-console
      return undefined;
    }
    ...
    if (!gl.getProgramParameter(prog, gl.LINK_STATUS)) {
      console.warn('CloudSky link:', gl.getProgramInfoLog(prog)); // eslint-disable-line no-console
      return undefined;
    }
    gl.useProgram(prog);
...
// A normal daytime sky, toned down from a glaring near-white/pure-white
// horizon and clouds to a softer, muted blue — still clearly a light sky,
// not the old dark stage, just not blinding.
const CASTERLY_PRESET_PROPS = {
  background: '#3d6f9e',
  baseColor: '#9dc2dc',
  accentColor: '#e8edf2',
  density: 100,
...
export default function CloudSky(props) {
  return <CloudSkyBase {...CASTERLY_PRESET_PROPS} {...props} />;
}
